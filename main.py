    ...
    for use_custom_prompt in [True]:#[False, True]:
        for model_name in MODEL_TYPE_REGISTRY.keys():
            for dataset_name in DATASET_LABEL_REGISTRY.keys():
                ...
                setattr(generation_args, 'split', 'train' if dataset_name in ('copa', 'rte', 'boolq', 'qnli') else 'test')
                setattr(args, 'split', 'train' if dataset_name in ('copa', 'rte', 'boolq', 'qnli') else 'test')
